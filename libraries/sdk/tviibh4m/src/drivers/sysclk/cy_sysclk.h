                                                                 ...
                                                                 uint32_t* feedBackFracDiv)
{
    ...
    for (uint32_t i_refDiv = lim->minRefDiv; i_refDiv <= lim->maxRefDiv; i_refDiv++)
    {
        ...
        for (uint32_t i_outDiv = lim->minOutputDiv; i_outDiv <= lim->maxOutputDiv; i_outDiv++)
        {
            ...
            int64_t error_tmp = ((uint64_t)targetOutFreq << (uint64_t)fracBitNum) - ((uint64_t)inFreq * tempFeedBackDivLeftShifted / ((uint64_t)i_refDiv * (uint64_t)i_outDiv));
            uint64_t error = (error_tmp < 0) ? -error_tmp : error_tmp;
            if (error < errorMin)
            {
